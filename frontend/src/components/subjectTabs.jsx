...
export const SubjectTabs = ({ data, activeSubject, onSubjectChange, onPracticeMode }) => {
  const iconMap = {
    ...
    <div className="display flex justify-between w-full">
      <div className="flex flex-wrap gap-4 mb-8">
        {uniqueSubjects.map((subject) => (
          <button
            key={subject}
            className={`tab-button flex items-center justify-center px-6 py-3 rounded-lg transition-all hover:cursor-pointer hover:bg-black hover:text-white ${subject === activeSubject
              ? "bg-black text-white"
