...
public class QuizController {
     ...
     * @param requestBody containing count, questionIds, subjectId, teacherId
     * @return the created quiz or error message
     */
    ...
    public ResponseEntity<?> createManualQuiz(@RequestBody Map<String, Object> requestBody) {
        try {
            ...
            // Extract question IDs from request (might be a List from JSON)
            ...
            responseBody.put("quiz_id", savedQuiz.getQuizId());
            responseBody.put("message", "Quiz created successfully");
            responseBody.put("requested_questions", selectedQuestionIds.size());
            responseBody.put("actual_questions", finalQuestionIds.size());
            
            if (finalQuestionIds.size() < count) {
                responseBody.put("warning", "Not enough questions available. Created quiz with " + finalQuestionIds.size() + " questions instead of " + count);
            }
    ...
    public ResponseEntity<?> createAutomaticQuiz(@RequestBody Map<String, Object> requestBody) {
        try {
            ...
            quiz.setSubjectId(subjectId);
            quiz.setMarks(totalMarks);
            quiz.setLevel(String.valueOf(level)); // Store numeric level (1,2,3,4) instead of string representation
            quiz.setTimelimit(timeLimit == null ? 30 : timeLimit);
            quiz.setType("Automatic");
            quiz.setQuestionIds(finalQuestionIds.toArray(new Integer[0]));
            
            // Set appropriate title and description based on level
            String levelDesc = (level == 4) ? "Mixed" : String.valueOf(level);
            quiz.setTitle(title == null ? "Auto Quiz - Level " + levelDesc : title);
            quiz.setDescription(description == null ? 
                (level == 4 ? "Automatically generated quiz with mixed difficulty levels" : 
                "Automatically generated quiz with difficulty level " + level) : description);
            ...
            responseBody.put("quiz_id", savedQuiz.getQuizId());
            responseBody.put("message", "Quiz created successfully");
            responseBody.put("requested_questions", count);
            responseBody.put("actual_questions", finalQuestionIds.size());
            responseBody.put("level", level);
            
            if (finalQuestionIds.size() < count) {
                responseBody.put("warning", "Not enough questions available at level " + level + ". Created quiz with " + finalQuestionIds.size() + " questions instead of " + count);
            }
        ...
        try {
            ...
            for (Integer qid : updatedQuestionIds) {
                ...
                if (question.isPresent()) {
                    ...
                    // If any of the provided question IDs doesn't exist, remove it from the list
                    updatedQuestionIds.remove(qid);
                }
            }
            ...
            quiz.setQuestionIds(updatedQuestionIds.toArray(new Integer[0]));
            quiz.setMarks(totalMarks);
            ...
            responseBody.put("new_count", updatedQuestionIds.size());
    ...
    public ResponseEntity<?> editQuizManually(
        ...
        try {
            ...
            // Store original question count for response
            int originalQuestionCount = quiz.getQuestionIds() != null ? quiz.getQuestionIds().length : 0;
            
            // Update the quiz
            quiz.setQuestionIds(finalQuestionIds.toArray(new Integer[0]));
            quiz.setMarks(totalMarks);
            quiz.setType("Manual"); // Set or confirm the quiz type as Manual
            ...
            if (finalQuestionIds.size() < count) {
                responseBody.put("warning", "Not enough questions available. Updated quiz with " + finalQuestionIds.size() + " questions instead of " + count);
            }
            
            return new ResponseEntity<>(responseBody, HttpStatus.OK);
            
        } catch (Exception e) {
            return handleInternalServerError(e, "updating quiz manually");
        }
    }
    ...
    // Global replacement of all catch blocks using a helper method
    private ResponseEntity<?> handleInternalServerError(Exception e, String operation) {
        return new ResponseEntity<>(new MessageResponse("Error " + operation + ": " + e.getMessage()),
